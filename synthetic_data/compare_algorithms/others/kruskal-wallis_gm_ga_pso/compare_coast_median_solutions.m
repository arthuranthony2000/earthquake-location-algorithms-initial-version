...
legend('Custo Médio Gauss-Newton com Multi-Start', 'Custo Médio Algoritmo Genético', 'Custo Médio Algoritmo de Otimização por Enxame de Partículas');
...
dados = [fmss_gm, fmss_ga, fmss_pso];
...
fprintf('H0: Não há diferença entre os custos das soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
fprintf('H1: Há diferença entre os custos das soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
fprintf('Valor-p: %.4f\n', p);
...
if p < alpha
    fprintf('Rejeitamos H0. Há diferença entre os custos das soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
    if norm(mss_ga - ss) < norm(mss_gm - ss)      
        if norm(mss_ga - ss) < norm(mss_pso - ss)
        ...
        if norm(mss_gm - ss) < norm(mss_pso - ss)
    ...
    fprintf('Não rejeitamos H0. Não há diferença entre os custos das soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
end
