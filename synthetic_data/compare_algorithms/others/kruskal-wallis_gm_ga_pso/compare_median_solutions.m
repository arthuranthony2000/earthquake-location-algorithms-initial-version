...
grupos = [ones(length(mss_gm), 1);  % Grupo 1: mss_gm
          ...
          3 * ones(length(mss_pso), 1)];  % Grupo 3: mss_pso

% Concatene os dados dos diferentes grupos
dados = [mss_gm, mss_ga, mss_pso];
...
fprintf('H0: Não há diferença entre as soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
fprintf('H1: Há diferença entre as solução médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
fprintf('Valor-p: %.4f\n', p);
...
if p < alpha
    fprintf('Rejeitamos H0. Há diferença entre as soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
    ...
    fprintf('Não rejeitamos H0. Não há diferença entre as soluções médias obtidas pelo Gauss-Newton com Multi-Start, Algoritmo Genético e Nuvem de Partículas.\n');
end
